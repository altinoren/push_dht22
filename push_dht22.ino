// Base code from https://github.com/szuecs/esp32-dh22-metrics/blob/master/esp32-dh22-metrics.ino

// A quick implementation collecting temp and humidity data, pushing to PushGateway periodically, ending up in Prometheus for
// monitoring home humidity. Resets every 10 minutes since my board locks up arbitrarily time to time and a 10 min reset seems OK
...
const char SSID[]     = "XX";    // your WiFi name
const char PASSWORD[] = "XX"; // your WiFi password
const char * HOST     = "192.168.1.240";  // pushGW: ip or dns
...
// prometheus metadata
String JOB = String("Temperature_Humidity");
String INSTANCE = String("Bedroom");
...
void loop()
{
  ...
  if (millis() > minute * 10) {esp_restart();}
